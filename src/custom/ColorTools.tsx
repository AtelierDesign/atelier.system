...
function EditableScale({ name, lightThemeConfig, darkThemeConfig }: EditableScaleProps) {
  ...
  const showAlphaValues = (document.querySelector('[data-alpha-scales]') as HTMLInputElement | null)?.checked ?? false;
  ...
  const showResetButton =
    curveRef.current.join() !== (isDarkTheme ? darkThemeConfig : lightThemeConfig).defaultCurve.join();
  ...
  const onCurveChange = React.useCallback(
    (newCurve: Curve) => {
      ...
      Array.from(Array(12)).forEach((_, index) => {
        ...
        const alphaValue = isDarkTheme && index === 0 ? 'hsl(0 0% 0% / 0)' : getAlphaColor(targetColor, backdropColor);
    ...
    <Box data-editable-scale={name} css={{ mb: '-$2', color: '$hiContrast' }}>
      <Flex css={{ ai: 'center', mr: '$1' }}>
        ...
        {showResetButton && (
          <Box
            ...
            onClick={event => {
              if (event.altKey) {
                document.querySelectorAll(`[data-reset-button]`).forEach(element => (element as HTMLElement).click());
                return;
              }
          ...
          onClick={event => {
            ...
            if (event.metaKey) {
              ...
              document.querySelectorAll(`[data-color-code]`).forEach(element => {
                ...
                const scaleName = parent.getAttribute('data-editable-scale')! + (showAlphaValues ? 'A' : '');
                ...
                parent.querySelector('[data-code-button]')!.setAttribute('data-show-check-icon', 'true');

                setTimeout(() => {
                  parent!.querySelector('[data-code-button]')!.removeAttribute('data-show-check-icon');
                }, 1000);
              });
      ...
      {!collapsed && (
        <Box>
          ...
          >
            {steps.map(step => {
              const variableName = showAlphaValues ? `--colors-${name}A${step}` : `--colors-${name}${step}`;
          ...
          <Editor curve={isDarkTheme ? darkThemeCurve : lightThemeCurve} onCurveChange={onCurveChange} />
        </Box>
      )}
    </Box>
  );
...
function Editor({ curve, onCurveChange }: EditorProps) {
  ...
  const inputValueIsValid = inputValueCurve.length === 4 && inputValueCurve.every(number => !isNaN(number));
...
function generateColors({ name, start, end, steps, hueCurve, chromaCurve, lumCurve }: ScaleSpec): Color[] {
